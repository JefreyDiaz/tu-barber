  ...
  return (
    <div className={`relative ${wrapperClassName ?? ''}`}>
      <input
        ...
        className={className ?? 'glass-input w-full px-4 py-3 pr-11 text-sm'}
      />
      <button
        type="button"
        tabIndex={-1}
        onClick={() => setVisible((v) => !v)}
        className="absolute right-3 top-1/2 -translate-y-1/2 rounded-md p-0.5 text-white/45 transition-colors hover:text-white/80"
