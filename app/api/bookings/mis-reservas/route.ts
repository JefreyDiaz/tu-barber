...
// GET: Obtener reservas del barbero logueado
export async function GET() {
  ...
  // Solo barberos y dueños pueden ver sus propias reservas
  if (session.user.role !== 'barbero' && session.user.role !== 'dueno') {
    return NextResponse.json(
      { success: false, error: 'Esta ruta es solo para barberos y dueños' },
      { status: 403 }
    );
  }
